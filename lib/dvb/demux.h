...
public:
	...
	RESULT createTSRecorder(ePtr<iDVBTSRecorder> &recorder, unsigned int packetsize = 188, bool streaming=false, bool sync_mode=false, bool is_streaming_output=false, bool use_scrambled_thread=true);
...
public:
	eDVBTSRecorder(eDVBDemux *demux, int packetsize, bool streaming, bool sync_mode = false, bool is_streaming_output = false, bool use_scrambled_thread = true);
	~eDVBTSRecorder();
