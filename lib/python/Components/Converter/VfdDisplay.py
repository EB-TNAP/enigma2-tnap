...
class VfdDisplay(Poll, Converter):
	def __init__(self, type):
		...
		if 'number' in self.type and 'clock' not in self.type:  # Only channel number
			...
			# Always use zero-padded hours (%H) regardless of the user's OSD
			# time-format setting.  7-segment VFD displays have a fixed-width
			# digit field; a single-digit hour (%-H) causes all characters to
			# shift left by one position, producing "hm:ms" instead of "hh:mm".
