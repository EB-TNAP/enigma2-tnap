...
class ScanSetup(ConfigListScreen, Screen, CableTransponderSearchSupport, TerrestrialTransponderSearchSupport):
	...
	def createConfig(self, frontendData):
		...
		if frontendData is not None:
			...
			if ttype == "DVB-S":
				...
				_freq_khz = frontendData.get("frequency", defaultSat["frequency"] * 1000)
	...
	def addSatTransponder(self, tlist, frequency, symbol_rate, polarisation, fec, inversion, orbital_position, system, modulation, rolloff, pilot, is_id, pls_mode, pls_code, t2mi_plp_id, t2mi_pid):
		...
		parm.modulation = modulation
		parm.system = system
		parm.frequency = int(round(frequency * 1000))
		...
		if self.DVB_type.value == "DVB-S":
			...
			if self.scan_type.value == "single_transponder":
				...
				if len(nimsats):
					...
					self.addSatTransponder(tlist, self.scan_sat.frequency.floatint / 1000.0,
				...
				if len(nimsats):
					...
					if len(tps) and len(tps) > self.preDefTransponders.index:
						...
						self.addSatTransponder(tlist, tp[1] / 1000.0, tp[2] // 1000, tp[3], tp[4], tp[7], orbpos, tp[5], tp[6], tp[8], tp[9], tp[10], tp[11], tp[12], tp[13], tp[14])
				removeAll = False
			elif self.scan_type.value == "single_satellite":
				sat = self.satList[index_to_scan][self.scan_satselection[index_to_scan].index]
	...
	def predefinedTranspondersList(self, orbpos):
		...
		if orbpos is not None:
			...
			compare = [
				0,  # DVB type
				self.scan_sat.frequency.floatint,  # 1 — already kHz
