...
class Wlan:
	...
	def getWirelessInterfaces(self):
		# Read from /proc/net/wireless instead of getWNICnames() which is unreliable
	...
	def setInterface(self, iface=None):
		self.iface = iface
		# TNAP: Reset oldInterfaceState when changing interfaces
		# This ensures proper state management for each scan
		self.oldInterfaceState = None
	...
	def getNetworkList(self):
		print(f"[Wlan] getNetworkList called for interface: {self.iface}")
		if self.oldInterfaceState is None:
			self.oldInterfaceState = iNetwork.getAdapterAttribute(self.iface, "up")
		print(f"[Wlan] Interface state: {self.oldInterfaceState}")
		if not self.oldInterfaceState:
			if not iNetwork.getAdapterAttribute(self.iface, "up"):
				print(f"[Wlan] Bringing up interface {self.iface}")
				...
				# TNAP: Short delay for interface initialization - required for reliable scanning
				# Yes this blocks UI briefly, but without it scans fail in NetworkWizard
				import time
				print("[Wlan] Waiting 1 second for interface to initialize...")
				time.sleep(1)

		ifobj = Wireless(self.iface) # a Wireless NIC Object
		print(f"[Wlan] Created Wireless object for {self.iface}")

		try:
			print("[Wlan] Starting WiFi scan...")
			scanresults = ifobj.scan()
			print(f"[Wlan] Scan completed: {len(scanresults) if scanresults else 0} networks found")
		except Exception as e:
			scanresults = None
			print(f"[Wlan] WiFi scan FAILED: {e}")
			import traceback
			traceback.print_exc()
		aps = {}
		if scanresults is not None:
			...
			for result in scanresults:
				...
				frequency_norm = ""
				try:
					freq = result.frequency.getFrequency()
					channel = frequencies.index(ifobj._formatFrequency(freq)) + 1
					# Determine frequency band (2.4 GHz or 5 GHz)
					if freq < 3000000000:  # Less than 3 GHz
						frequency_norm = "2.4 GHz"
					else:
						frequency_norm = "5 GHz"
				except:
					channel = "Unknown"
					frequency_norm = ""
